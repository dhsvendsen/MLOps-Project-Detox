    ...
    with torch.no_grad():
        ...
        for i, (tokens, mask, labels) in enumerate(test_set):
            ...
            if labels.sum() > 0:
                ...
                label_specific += (correct_1s.sum() / labels.sum()).type(
                    torch.FloatTensor
                )
                correct_rows = correct_1s.sum(axis=1) > 0
                toxic_rows = labels.sum(axis=1) > 0
                any_toxicity += (correct_rows.sum() / toxic_rows.sum()).type(
                    torch.FloatTensor
                )

        print(
            f'Percentage of all "1s" caught: {label_specific.item()*100/len(test_set)}%'
        )
        print(
            f"Percentage toxic (any kind) comments caught: {any_toxicity.item()*100/len(test_set)}%"
        )
